...
from client_rules import (
    extract_client_rule_values, extract_client_field_examples,
...
def get_oauth_flow():
    ...
    flow = _auth.build_oauth_flow(
        secrets_path=secrets_path,
        redirect_uri_override=os.environ.get("OAUTH_REDIRECT_URI", ""),
        config_value_func=_get_config_value,
    )
    if flow is None:
...
def get_ga4_accounts_structure(creds):
    try:
        return ga4_service.get_account_summaries(creds)
...
def get_top_traffic_sources(property_id, creds):
    try:
        return ga4_service.get_top_traffic_sources(property_id, creds)
    except Exception as e:
...
def get_top_traffic_mediums(property_id, creds):
    try:
        return ga4_service.get_top_traffic_mediums(property_id, creds)
    except Exception as e:
...
def get_source_medium_pairs(property_id, creds):
    try:
        return ga4_service.get_source_medium_pairs(property_id, creds)
    except Exception as e:
